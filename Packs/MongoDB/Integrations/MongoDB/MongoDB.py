...
class Client:
    def __init__(
        ...
        self._client = MongoClient(urls, username=username, password=password, ssl=ssl)
    ...
    def query(self, collection: str, query: dict) -> List[dict]:
        collection_obj = self.get_collection(collection)
        entries = collection_obj.find(query)
...
def convert_object_id_to_str(entries: List[ObjectId]) -> List[str]:
    ...
    Examples:
        ...
        ['5e4412f230c5b8f63a7356ba']
    """
    return [str(object_id) for object_id in entries]
...
def search_query(
    client: Client, collection: str, query: str, **kwargs
) -> Tuple[str, dict, list]:
    # test if query is a valid json
    try:
        query_json = json.loads(query)
        raw_response = client.query(collection, query_json)
        readable_outputs = tableToMarkdown(
            f"Total of {len(raw_response)} found in MongoDB collection `{collection}` with query: {query}:",
...
def main():
    ...
    client = Client(
        ...
        params["database"],
        not demisto.params().get("insecure", False),
    )
    commands = {
